...
void BigFloatRep :: fromString(const char *str, extLong prec ) {
  ...
  // std::cout << "fromString prec:" << prec << std::endl;
  if (prec.isInfty())
...
std::istream& BigFloatRep :: operator >>(std::istream& i) {
  ...
  for (; (isdigit(c) && !(i.eof())) || (!d && c=='.') ||
       (!e && ((c=='e') || (c=='E'))) || (!s && (c=='-' || c=='+')); i.get(c)) {  /// terminate if reaches EOF
  ...
  if (i.eof()) {
    ...
    i.clear();  /// restore to pointing at EOF but no eofbit set
    return i;
  }
  i.putback(c);
  fromString(str.c_str());
