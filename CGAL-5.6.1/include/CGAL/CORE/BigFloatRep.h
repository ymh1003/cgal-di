...
private:
  ...
  // void fromString(const char *p, extLong  prec = std::numeric_limits<double>::max_digits10+50);
...
inline std::ostream& BigFloatRep::operator<<(std::ostream& o) const {
  ...
  BigFloatRep::DecimalOutput r = toDecimal(get_static_defBigFloatOutputDigits(), sci);
  if (r.sign == -1)
    o << "-";
  o << r.rep;
  return o;
