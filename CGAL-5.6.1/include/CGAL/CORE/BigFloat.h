...
public:
  ...
  std::string toString(long prec=get_static_defBigFloatOutputDigits(), bool sci=false) const {
    std::cout << "output digits " << prec << std::endl;
...
inline std::ostream& operator<< (std::ostream& o, const BigFloat& x) {
  // std::cout << typeid(x.getRep()).name() << std::endl;
  x.getRep().operator<<(o);
  return o;
}
/// IO stream operator>>
...
/// specified for running Alpha_shapes_2
inline bool operator> (const BigFloat& x, const int& y) {
  return x.cmp(BigFloat(y)) > 0;
}
inline bool operator< (const BigFloat& x, const int& y) {
  return x.cmp(BigFloat(y)) < 0;
}
...
inline bool operator< (const BigFloat& x, const double& y) {
  return x.cmp(BigFloat(y)) < 0;
}
inline bool operator> (const BigFloat& x, const double& y) {
  return x.cmp(BigFloat(y)) > 0;
...
inline BigFloat operator+ (const BigFloat& x, const int& y) {
  return x + BigFloat(y);
}
inline BigFloat operator+ (const int& x, const BigFloat& y) {
  return BigFloat(x) + y;
}
inline BigFloat operator- (const BigFloat& x, const int& y) {
  return x - BigFloat(y);
  ...
  class To_interval
      ...
      public:
        std::pair<double, double> operator()( const Type& x ) const {
          double x_d = doubleValue(x);
          if (x > x_d) return std::make_pair(x_d, std::nextafter(x_d, DBL_MAX));
          if (x == x_d) return std::make_pair(x_d, x_d);
          if (x < x_d) return std::make_pair(std::nextafter(x_d, -DBL_MAX), x_d);
        }
      };
