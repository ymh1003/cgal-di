...
inline bool operator> (const Expr& x, const int& y) {
  return x.cmp(Expr(y)) > 0;
}
inline bool operator< (const Expr& x, const int& y) {
  return x.cmp(Expr(y)) < 0;
}
...
inline bool operator< (const Expr& x, const double& y) {
  return x.cmp(Expr(y)) < 0;
}
inline bool operator> (const Expr& x, const double& y) {
  return x.cmp(Expr(y)) > 0;
...
inline Expr operator+ (const Expr& x, const int& y) {
  return x + Expr(y);
}
inline Expr operator+ (const int& x, const Expr& y) {
  return Expr(x) + y;
}
inline Expr operator- (const Expr& x, const int& y) {
  return x - Expr(y);
...
inline Expr operator* (const Expr& x, const BigInt& y) {
  return x * Expr(y);
}
inline Expr operator/ (const Expr& x, const int& y) {
  return x / Expr(y);
