...
private:
  ...
  typedef std::multimap< Type_of_alpha, Face_handle, std::less<Type_of_alpha> >  Interval_face_map;
  ...
  typedef std::vector<std::string> Warning_buffer;
...
private:
  ...
  Warning_buffer _warning_buffer; 
...
public:
  void output_buffer()
    {
      if (!(_warning_buffer.empty())) {
        std::cerr << "----------- Buffered Warnings -----------" << std::endl;
        for (const std::string& warning : _warning_buffer) {
          std::cerr << warning << std::endl;
        }
        std::cerr << "------------------------------------------" << std::endl;
        _warning_buffer.clear();
      } // flush if want one-time warnings for multiple changes of alpha-values
    }
  ...
  Type_of_alpha set_alpha(const Type_of_alpha& alpha)
    {
      ...
      for(Finite_faces_iterator fit = faces_begin();
          fit != faces_end();
          ++fit){

        if(classify(fit) == INTERIOR){
          Triangle_2 t = this->triangle(fit);
          i++;
      ...
      for(Edge_iterator eit = edges_begin();
        ...
        // print regular edge
          j++;
          std::cout << "Regular edge " << j << std::endl;
          std::cout << this->segment(*eit) << std::endl;
          break;
        case SINGULAR :
        // print singular edge
          k++;
          std::cout << "Singular edge " << k << std::endl;
          std::cout << this->segment(*eit) << std::endl;
          break;
        default:
        // print normal edge
          e++;
          std::cout << "Normal edge " << e << std::endl;
          std::cout << this->segment(*eit) << std::endl;
        }
      }
      std::cout << "Triangles: " << i << std::endl;
      std::cout << "Regular Edges: " << j << std::endl;
      std::cout << "Singular Edges: " << k << std::endl;
      std::cout << "Normal Edges: " << e << std::endl;
      std::cout << "------------------------------------------" << std::endl;
...
public:
  ...
  Classification_type  classify(const Face_handle& f,
                                const Type_of_alpha& alpha) const
    {
      ...
      // return std::less_equal<Type_of_alpha>{}(find_interval(f), alpha) ?
...
Alpha_shape_2<Dt,EACT>::initialize_alpha_spectrum()
{
  ...
  typename Interval_edge_map::iterator
    ...
    // std::cout<<(*edge_it).first.first<<std::endl;
    // std::cout<<_interval_face_map.size()<<std::endl;
    // std::cout<<_interval_edge_map.size()<<std::endl;
    ...
    { 
      if (face_it != _interval_face_map.end() &&
          (edge_it == _interval_edge_map.end() ||
           ((*face_it).first < (*edge_it).first.first)))  /// 1
          // (std::less<Type_of_alpha>{}( (*face_it).first, (*edge_it).first.first) )))
        {
          // std::cout << "{" << std::endl;
          // std::cout << _alpha_spectrum.back() << std::endl;
          // std::cout << _alpha_spectrum.back().isExact();
          // std::cout << "<" << std::endl;
          // std::cout << (*face_it).first << std::endl;
          // std::cout << (*face_it).first.isExact();
          // std::cout << "}" << std::endl;

          if (((_alpha_spectrum.empty() ||
                _alpha_spectrum.back() < (*face_it).first)) &&  /// 2 ***
                // std::less<Type_of_alpha>{}( _alpha_spectrum.back(), (*face_it).first ))) &&
        ...
        {
          if (((_alpha_spectrum.empty() ||
                _alpha_spectrum.back() < (*edge_it).first.first)) &&  /// 3
                // std::less<Type_of_alpha>{}( _alpha_spectrum.back(), (*edge_it).first.first ))) &&
  ...
  while (edge_it != _interval_edge_map.end())
    {
      if (((_alpha_spectrum.empty() ||
            _alpha_spectrum.back() < (*edge_it).first.first))&&  /// 4
            // std::less<Type_of_alpha>{}( _alpha_spectrum.back(), (*edge_it).first.first)) )&&
          (((*edge_it).first.first) > Type_of_alpha(0)))
        _alpha_spectrum.push_back((*edge_it).first.first);
      edge_it++;
  ...
  while (face_it != _interval_face_map.end())
    {
      if (((_alpha_spectrum.empty() ||
            _alpha_spectrum.back() < (*face_it).first))&&  /// 5
            // std::less<Type_of_alpha>{} (_alpha_spectrum.back(), (*face_it).first )))&&
          ((*face_it).first > Type_of_alpha(0)))
        _alpha_spectrum.push_back((*face_it).first);
      face_it++;
  ...
  // for (const Type_of_alpha& a : _alpha_spectrum) {
  //   std::cout << a << std::endl;
  // }

  // std::cout << _alpha_spectrum.size() << std::endl;

  for (const std::string& warning : CGAL::warningBuffer) {
    _warning_buffer.push_back(warning);
  }

  CGAL::clearBuffer();
  CGAL::bufferWarnings = false;
                            ...
                            const Type_of_alpha& alpha) const
{
  ...
  // if (std::less<Type_of_alpha>{}(alpha, interval.second))

    {
      if (get_mode() == REGULARIZED ||
          interval.first == UNDEFINED ||
          alpha < interval.first)
          // std::less<Type_of_alpha>{}(alpha, interval.first))
    ...
    {   // alpha >= interval.second
      if (interval.third == Infinity ||
          ...
          // std::less<Type_of_alpha>{}(alpha, interval.third))
        return REGULAR;
